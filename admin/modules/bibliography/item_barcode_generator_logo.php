...
if (isset($_GET['action']) AND $_GET['action'] == 'print') {
  ...
  // $item_q = $dbs->query('SELECT b.title, b.classification, i.item_code, i.call_number FROM item AS i
  ...
  $html_str .= '.labelStyle { padding-left:30px; width:9.5cm; height: 6cm; text-align: center; ; margin: '.$sysconf['print']['barcode']['barcode_items_margin'].'cm; border: '.$sysconf['print']['barcode']['barcode_border_size'].'px solid #000000;}'."\n";
  $html_str .= '.labelStyle1 { width: 9.5cm; height: 6cm; text-align: center; margin: '.'0'.'cm; }'."\n";
  ...
  foreach ($chunked_barcode_arrays as $barcode_rows) {
		...
		$html_str .= '<td valign="top">';
          ...
          //Mengatur Logo Barcode
				    if ($sysconf['print']['barcode']['barcode_cut_title']) {         			
				  	$html_str .= '<td rowspan="2" style="solid #000000; padding-left:px; margin-top: 20px; padding-right:0cm;padding-bottom:4cm;"><div class="barcode_rotate_2">'.''.'<img height="40px" width="40px" src="'.SWB.'files/membercard/'.$sysconf['print']['membercard']['logo'].'" />';
					} else { $html_str .= $barcode[0]; }
					//Mengatur Kolom Header Call Number
				    $html_str .= '</td>';
         $html_str .=
         '<td style="background-color:'
         .$warna.'; border-bottom:0px solid #000000;"><div style="font-size: 12pt; ; padding-left: 25; vertical-align: text-top; font-weight:bold;">'
         .($sysconf['print']['barcode']['barcode_header_text1']).'</div><div style="font-size: 11pt;padding-left:40px; font-weight:none;">'
         .($sysconf['print']['barcode']['barcode_header_text2']).'</div><div style="font-size: 9pt; padding-left: 30px">'
         .($sysconf['print']['barcode']['barcode_header_text3']).'</div>';
				  $html_str .= '</td>';
				  $html_str .= '<tr><td>';
